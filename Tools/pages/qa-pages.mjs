...
async function auditHttpSurface(base) {
  ...
  assert(preyManifestJson.defaultAspectRatio === '16:9' && Array.isArray(preyManifestJson.slots) && preyManifestJson.slots.length === 4, 'Prey illustrated journal manifest slot contract mismatch');
  assert(preyManifest.text.includes('prey-page-02-a-rooftop-helicopter.png') && preyManifest.text.includes('prey-page-03-a-mimic-paranoia.png') && preyManifest.text.includes('prey-page-03-b-crew-terminal-trace.png') && preyManifest.text.includes('prey-page-04-a-lobby-wrench-mimic.png'), 'Prey manifest missing expected filenames');
