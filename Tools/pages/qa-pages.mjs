...
async function auditHttpSurface(base) {
  ...
  assert(preyJourney.text.includes('Fake Morning') && preyJourney.text.includes('Test Rooms') && preyJourney.text.includes('Escape') && preyJourney.text.includes('Office') && preyJourney.text.includes('Looking Glass') && preyJourney.text.includes('Art Queue'), 'Prey journey required journal pages missing');
  ...
  assert((preyJourney.text.match(/data-prey-slot="/g) || []).length === 4, 'Prey journey should expose exactly four visible Prey image slots');
  ...
  assert((preyJourney.text.match(/<div class="evidence-slot[^"]*" data-image-ratio="9:16"/g) || []).length === 1, 'Prey journey should expose exactly one visible 9:16 portrait image frame');
  ...
  assert(preyManifestJson.defaultAspectRatio === '16:9' && Array.isArray(preyManifestJson.slots) && preyManifestJson.slots.length === 5, 'Prey illustrated journal manifest slot contract mismatch');
  assert(preyManifest.text.includes('prey-page-02-a-rooftop-helicopter.png') && preyManifest.text.includes('prey-page-03-a-mimic-paranoia.png') && preyManifest.text.includes('prey-page-03-b-crew-terminal-trace.png') && preyManifest.text.includes('prey-page-04-a-lobby-wrench-mimic.png') && preyManifest.text.includes('prey-page-05-a-office-looking-glass.png'), 'Prey manifest missing expected filenames');
