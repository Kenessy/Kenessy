...
async function auditViewport(browser, base, buildId, viewport) {
  ...
  assert(metrics.metaChips.count === 5 && metrics.metaChips.labels === 'Status|Evidence|Confidence|Spoilers|Risk' && metrics.metaChips.values === 'Complete|Full run / veteran memory|Max / replay-verified|Layered policy|Ending reconstructed', `${viewport.name} meta chip content mismatch ${JSON.stringify(metrics.metaChips)}`);
