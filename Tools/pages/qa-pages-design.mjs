...
async function auditViewport(browser, base, buildId, viewport) {
  ...
  const metrics = await page.evaluate(() => {
    ...
    const signalBlocks = {
      ...
      inspectAfterIdentity: Boolean(document.querySelector('.scr-hero-copy .scr-identity + .scr-inspect-panel')),
  ...
  assert(metrics.signalBlocks.score === 7 && metrics.signalBlocks.axis === 5 && metrics.signalBlocks.arc === 9 && metrics.signalBlocks.inspect === 7 && metrics.signalBlocks.inspectInScorePanel === 0 && metrics.signalBlocks.inspectAfterIdentity && metrics.signalBlocks.meterScreens === 28, `${viewport.name} score-screen signal block coverage mismatch ${JSON.stringify(metrics.signalBlocks)}`);
