...
async function auditDesignArtifacts(base) {
  ...
  assert(root.text.includes('.prey-card') && root.text.includes('plater-game-reports/games/prey/') && root.text.includes('plater-game-reports/games/prey/journey/') && root.text.includes('Field journal ready'), 'portfolio homepage Prey card/journal styling link is missing');
  ...
  assert(root.text.includes('.journal-contract') && root.text.includes('.journal-file-queue') && root.text.includes('Page 02 logged') && root.text.includes('qb-page-02-a-airlock-threshold.png') && root.text.includes('2 photos wired'), 'portfolio homepage journal photo-evidence contract/drop queue styling is missing');
  assert(root.text.includes('.journal-asset-links') && root.text.includes('Photo manifest'), 'portfolio homepage journal asset links styling is missing');
  ...
  assert(root.text.includes('State</b> Live draft') && root.text.includes('exact 16:9 filenames auto-wire into visible photo evidence slots'), 'portfolio homepage Quantum Break state/journal handoff copy is missing');
  ...
  assert(quantumBreakJourney.text.includes('Fullscreen Journal') && quantumBreakJourney.text.includes('Play-it-together scrapbook reader'), 'Quantum Break journey fullscreen scrapbook reader copy is missing');
  ...
  assert(quantumBreakJourney.text.includes('.dossier-page') && quantumBreakJourney.text.includes('.scrapbook-board') && quantumBreakJourney.text.includes('.photo-card'), 'Quantum Break journey dossier scrapbook layout is missing');
  ...
  assert(quantumBreakJourney.text.includes('.case-block') && quantumBreakJourney.text.includes('.field-note') && quantumBreakJourney.text.includes('.marker-title'), 'Quantum Break journey scrapbook note styling is missing');
  assert(quantumBreakJourney.text.includes('The Machine Breaks') && quantumBreakJourney.text.includes('First Stutter') && quantumBreakJourney.text.includes('frozen-time state lands') && quantumBreakJourney.text.includes('combat becomes readable'), 'Quantum Break journey replay-note page 02 copy is missing');
  ...
  assert(quantumBreakJourney.text.includes('.photo-slot') && quantumBreakJourney.text.includes('.photo-slot-ready .photo-placeholder{display:none}') && quantumBreakJourney.text.includes('.photo-paper'), 'Quantum Break journey photo evidence frame styling is missing');
  assert(quantumBreakJourney.text.includes('data-qb-slot="page-02-a"') && quantumBreakJourney.text.includes('data-qb-slot="page-02-c"') && !quantumBreakJourney.text.includes('data-qb-slot="page-02-d"'), 'Quantum Break journey visible photo slot markers are wrong');
  assert(!quantumBreakJourney.text.includes('.panel-frame') && !quantumBreakJourney.text.includes('IMAGE SLOT') && !quantumBreakJourney.text.includes('<b>4</b> image slots'), 'Quantum Break journey still contains old panel-frame slot styling');
  ...
  assert(quantumBreakJourney.text.includes('Generation Brief') && quantumBreakJourney.text.includes('qb-page-02-c-core-detonation.png'), 'Quantum Break journey image brief styling is missing');
  assert(quantumBreakJourney.text.includes('Shared style contract') && quantumBreakJourney.text.includes('Dossier photo language'), 'Quantum Break journey prompt-pack styling/copy is missing');
  assert(quantumBreakJourney.text.includes('No logos') && quantumBreakJourney.text.includes('No fake text'), 'Quantum Break journey image guardrail prompt deck is missing');
  ...
  assert(quantumBreakJourney.text.includes('coherent cinematic sci-fi dossier-photo style') && quantumBreakJourney.text.includes('no fake UI') && quantumBreakJourney.text.includes('no captions'), 'Quantum Break journey prompt style guardrails are missing');
  assert(quantumBreakJourney.text.includes('build auto-wires any matching image file into its photo evidence slot'), 'Quantum Break journey auto-wiring contract is missing');
  ...
  assert(quantumBreakReport.text.includes('.evidence-ledger .aud:before') && quantumBreakReport.text.includes('.review-note-section .field-note:before') && quantumBreakReport.text.includes('.reader-tags:before'), 'Quantum Break report evidence/reviewer polish styling is missing');
  assert(quantumBreakReport.text.includes('class="section live-evidence-section"') && quantumBreakReport.text.includes('class="ledger evidence-ledger"') && quantumBreakReport.text.includes('class="section review-note-section"'), 'Quantum Break report evidence/reviewer section classing is missing');
  ...
  assert(preyReport.text.includes('Field journal ready') && preyReport.text.includes('Open Journal') && preyReport.text.includes('journey/'), 'Prey report journal link/copy is missing');
  ...
  assert(preyJourney.text.includes('Fake Morning') && preyJourney.text.includes('Test Rooms') && preyJourney.text.includes('Escape') && preyJourney.text.includes('Office') && preyJourney.text.includes('Looking Glass') && preyJourney.text.includes('Teleconferencing Keycard') && preyJourney.text.includes('pistol pickup') && preyJourney.text.includes('Hardware Labs') && preyJourney.text.includes('Ballistics Lab') && preyJourney.text.includes('Recycler Charge') && preyJourney.text.includes('Dr. Calvino') && preyJourney.text.includes("Calvino's body") && preyJourney.text.includes('Q-Beam') && preyJourney.text.includes('Josh Dalton') && preyJourney.text.includes('Probe activated') && preyJourney.text.includes('Labs cleared') && preyJourney.text.includes('Looking Glass restored') && preyJourney.text.includes('Return to Morgan office') && preyJourney.text.includes('Art Queue'), 'Prey journey tab copy is missing');
  assert((preyJourney.text.match(/data-prey-slot="/g) || []).length === 6, 'Prey journey should expose exactly six Prey image slots');
  assert((preyJourney.text.match(/<div class="evidence-slot[^"]*" data-image-ratio="16:9"/g) || []).length === 3, 'Prey journey should expose exactly three visible 16:9 frames');
  assert((preyJourney.text.match(/<div class="evidence-slot[^"]*" data-image-ratio="9:16"/g) || []).length === 3, 'Prey journey should expose exactly three visible 9:16 portrait frames');
  assert(preyJourney.text.includes('prey-page-02-a-rooftop-helicopter.png') && preyJourney.text.includes('prey-page-03-a-mimic-paranoia.png') && preyJourney.text.includes('prey-page-03-b-crew-terminal-trace.png') && preyJourney.text.includes('prey-page-04-a-lobby-wrench-mimic.png') && preyJourney.text.includes('prey-page-05-a-office-looking-glass.png') && preyJourney.text.includes('prey-page-06-a-teleconferencing-keycard.png') && preyJourney.text.includes('prey-page-08-a-calvino-exterior.png'), 'Prey journey image handoff filenames missing');
  ...
  assert(!/letter-spacing:-/i.test(preyJourney.text), 'Prey journey CSS has negative letter spacing');
  const preyManifest = await fetchText(url(base, preyFlightRecorderManifestPath));
  assert(preyManifest.text.includes('"game": "Prey"') && preyManifest.text.includes('"promptVersion": "prey-illustrated-journal-v1"'), 'Prey illustrated journal manifest metadata missing');
  assert(preyManifest.text.includes('"defaultAspectRatio": "16:9"') && preyManifest.text.includes('"styleRules"'), 'Prey illustrated journal manifest 16:9/style contract missing');
  ...
  assert(panelManifest.text.includes('"styleRules"') && panelManifest.text.includes('dark dossier photo language'), 'Quantum Break panel manifest styleRules missing');
