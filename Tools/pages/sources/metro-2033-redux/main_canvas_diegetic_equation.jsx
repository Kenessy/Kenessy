...
function ScoreTile({ item, mode }) {
  ...
  return (
    <article className="scr-score-tile" style={{ '--tone': item.color, '--off': inactiveMeterRow(item.color, item.name === 'Danger') }}>
      ...
      <div className="scr-score-meter-box scr-signal-block">
...
function AxisRow({ axis }) {
  return <article className="scr-axis-row" style={{ '--tone': axis.color }}><div className="scr-axis-title"><small>ALERT Axis</small><h3>{axis.name}</h3><span>{axis.grade} · {axis.score}/20</span></div><div className="scr-axis-body"><SegmentBar value={axis.score} color={axis.color} /><p>{axis.text}</p></div><div className="scr-d20-wrap scr-signal-block"><span className="scr-signal-kicker">Axis</span><div className="scr-d20 scr-signal-value">{axis.score}</div><small className="scr-signal-note">/20</small></div></article>;
}
...
function EvidenceCard({ arc }) {
  ...
  return <article className="scr-evidence-card" style={{ '--tone': arc.color }}><div className="scr-evidence-marker scr-signal-block"><span className="scr-signal-kicker">ARC</span><b className="scr-signal-value">{arc.id}</b><small className="scr-signal-note">Dossier</small></div><div><div className="scr-evidence-title-row"><h3>{arc.title}</h3><span className={`scr-spoiler ${spoilerClass}`}>Spoiler-{arc.spoiler}</span></div><div className="scr-evidence-cells"><EvidenceCell label="Clean observation" text={arc.observation} /><EvidenceCell label="What it proves" text={arc.proof} /><EvidenceCell label="Caveat" text={arc.caveat} muted /></div><div className="scr-evidence-impact">▸ Axis impact · {arc.impact}</div></div></article>;
}
