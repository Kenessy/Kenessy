  ...
  return (
    <section className="scr-hero">
      <div className="scr-hero-grid">
        ...
        <DiegeticVerdict />
        <div className="scr-hero-inspect-row">
  ...
  return (
    <aside className="scr-verdict scr-verdict-simple">
      <div className="scr-score-panel scr-score-panel-meter scr-score-panel-simple" style={{ '--tone': COLORS.amber }}>
        ...
        <SignalMeterBlock className="scr-main-score-meter" value={REVIEW.score} color={COLORS.amber} activeRows={activeRows} />
      </div>
    </aside>
  );
}
...
function InspectFitPanel({ rows }) {
  return (
    <div className="scr-inspect-panel">
      ...
      <div className="scr-inspect-list">{rows.map((row) => <InspectFitTile key={row.name} row={row} />)}</div>
    </div>
  );
}

function InspectFitTile({ row }) {
  const activeRows = Math.max(0, Math.min(20, Math.round(row.score * 2)));

  return (
    <article className="scr-inspect-tile" style={{ '--tone': row.color }}>
      <div className="scr-tile-outside-head" style={{ color: row.color }}>
        <div className="scr-tile-letter">{row.letter}</div>
        <div className="scr-tile-name">{row.name}</div>
        <div className="scr-tile-desc">{row.hint}</div>
      </div>
      <SignalMeterBlock className="scr-inspect-meter" value={row.score} color={row.color} activeRows={activeRows} />
      <div className="scr-tile-outside-grade" style={{ color: row.color }}>{row.score}/10 fit</div>
    </article>
  );
}
...
function SignalMeterBlock({ value, color, activeRows = 20, fillFrom = 'bottom', danger = false, label, note, className = '' }) {
